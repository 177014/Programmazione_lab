            ...
            with open(self.name, 'r') as csvfile:
                ...
                for row in my_file:
                    ...
                    try:
                        ...
                        time_series.append([str(date), int(passengers)])                            #Memorizza la data e il valore dei passeggeri validi
                    except ValueError:
                ...
                for i in range(0,len(years_months) - 1):
                    if int(years_months[i][1]) > 12:
                        raise ExamException("Non posso esserci più di 12 mesi")                     #Genera un'eccezione se un mese è maggiore di 12

                    if years_months[i+1][0] < years_months[i][0]:
                        raise ExamException("Gli anni del file csv non sono in ordine")             #Genera un'eccezione se gli anni non sono in ordine crescente

                    for j in range(1,len(years_months) - 1 - i):
                        if time_series[i][0] == time_series[i+j][0]:
                            raise ExamException("Ci sono date duplicate")                           #Genera un'eccezione se ci sono date duplicate

        except FileNotFoundError:
            raise ExamException("File non trovato")                                                 #Genera un'eccezione se il file non viene trovato
...
def compute_avg_monthly_difference(time_series, first_year, last_year):
    ...
    if not time_series:
        raise ExamException("time_series vuota")                                                    #Genera un'eccezione se time_series è vuota

    try:
        first_year = int(first_year)
        last_year = int(last_year)
    except ValueError:
        raise ExamException("Gli anni inseriti non sono numerici interi")                           #Genera un'eccezione se gli anni inseriti non sono numeri interi

    if first_year > last_year:
        raise ExamException("Il primo anno deve essere minore dell'ultimo")                         #Genera un'eccezione se il primo anno è maggiore dell'ultimo

    monthly_diffs = []
    for i in range(12):
        monthly_diffs.append([])                                                                    #Inizializza una lista per ogni mese per memorizzare le differenze
    ...
    for i in range(len(time_series)):
        ...
        if first_year <= year < last_year:
            ...
            if next_year_same_month is not None:
                ...
                monthly_diffs[month - 1].append(diff)                                               #Lista contenente la differenza tra i passeggeri

    result = []
    for i in monthly_diffs:
        if i:
